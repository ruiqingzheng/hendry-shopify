...
app.post('/webhook/product-updated', async (req, res) => {
  try {
    ...
    if (!verifyWebhook(body, hmac_header)) {
      // 数据校验失败则退出, 但依然要回复 webhook 服务器
      console.log('Data hmac hash error!')
      res.status(200).send('webhook works!')
      return
    }
    ...
    console.log(`product ${webhookData.title} tags updated!!!`);
  } catch (error) {
    console.log('webhook/product-updated error: ', error.message)
  } finally {
    res.status(200).send('webhook works!')
  }
})
